...
Private Function afficher_infos() As Integer
    ...
    If GetSetting(App.EXEName, "Section", "NumUtil") <> "" Then
        ...
        If p_numUtil > 0 Then
            ...
            'Identifiant
            If Odbc_RecupVal("SELECT UAPP_Code FROM UtilAppli WHERE UAPP_UNum=" & p_numUtil, scode) = P_ERREUR Then
...
Private Sub mnuKW_Click()
    ...
    Shell "C:\Program Files\Internet Explorer\iexplore.exe " & p_cheminphp & "/pident.php?s_vers_conf=_acc&?V_util=" & snumutil, vbMaximizedFocus
...
'appelée quand l'utilisateur clique sur le menu contextuel Quitter
    ...
    reponse = MsgBox("Souhaitez-vous réellement quitter KaliAlert ?" & vbLf & vbLf _
