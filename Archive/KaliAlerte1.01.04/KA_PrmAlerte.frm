...
Private Function verif_ident() As Integer
    ...
    Else
        If rs("UAPP_MotPasse").Value <> "" Then
            If (rs("UAPP_TypeCrypt").Value = "kalidoc" Or rs("UAPP_TypeCrypt").Value = "") And STR_Decrypter(rs("UAPP_MotPasse").Value) <> UCase(rs("UAPP_MotPasse").Value) Then
                bad_util = True
            ElseIf rs("UAPP_TypeCrypt").Value = "kalidocnew" And STR_Decrypter_New(rs("UAPP_MotPasse").Value) <> UCase(rs("UAPP_MotPasse").Value) Then
                bad_util = True
            ElseIf rs("UAPP_TypeCrypt").Value = "md5" And rs("UAPP_MotPasse").Value <> oMD5.MD5(rs("UAPP_MotPasse").Value) Then
                bad_util = True
            Else
