...
Begin VB.Form KA_Alerte 
   ...
   Begin VB.Frame frmconnect 
      ...
      Begin VB.Label Label4 
         ...
         Caption         =   "Vérifiez votre connexion ou contactez l'administrateur"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
      ...
      Begin VB.Label Label3 
         Alignment       =   2  'Center
         BackStyle       =   0  'Transparent
         Caption         =   "Impossible d'accéder aux informations"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Frame frmDet 
      ...
      Begin VB.CommandButton cmd 
         ...
         Index           =   4
         Left            =   6480
         Picture         =   "KA_Alerte.frx":5D0E
         Style           =   1  'Graphical
         TabIndex        =   59
         ToolTipText     =   "Accès à KaliDoc"
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   650
      End
      Begin VB.CommandButton cmd 
         ...
         Index           =   5
         Left            =   7920
         Picture         =   "KA_Alerte.frx":60BA
         Style           =   1  'Graphical
         TabIndex        =   60
         ToolTipText     =   "Accès à KaliGdR"
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   650
      End
   ...
   Begin ComctlLib.ImageList ImageList 
      ...
      BeginProperty Images {0713E8C2-850A-101B-AFC0-4210102A8DA7} 
         NumListImages   =   6
         BeginProperty ListImage1 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "KA_Alerte.frx":6D28
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "KA_Alerte.frx":71F2
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "KA_Alerte.frx":76BC
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "KA_Alerte.frx":7B86
            Key             =   ""
         EndProperty
         BeginProperty ListImage5 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "KA_Alerte.frx":8050
            Key             =   ""
         EndProperty
         BeginProperty ListImage6 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "KA_Alerte.frx":85A2
            Key             =   ""
         EndProperty
      EndProperty
   End
...
Private Const CMD_KD = 4
Private Const CMD_KR = 5
Private Const CMD_KW = 6
...
Public g_loffset As Long
Private g_ltimer As Long
...
Private Function charger_identifant() As Integer
    ...
    Label1.Caption = "KaliAlerte - V1.01.04 - " & UCase(Left$(sprenom, 1) & "." & snom)
...
Private Function charger_kform() As Integer
    ...
    sql = "SELECT CATF_Nom, FOR_Lib, DONEC_FORNum, DONEC_DONNum, DONEC_NumEtape, DONEC_Vu " _
        & "FROM CategForm, Formulaire, Donnees_Encours " _
        & "WHERE DONEC_FORNum = FOR_Num " _
        & "AND FOR_CATFNum = CATF_Num " _
        & "AND DONEC_UNum=" & p_numUtil _
        & " ORDER BY DONEC_Date Desc, DONEC_Num Desc " _
        & " OFFSET " & g_loffset & " LIMIT " & NB_LIMIT
'        & " AND DONEC_Vu = 'f'"
    If Odbc_SelectV(sql, rs) = P_ERREUR Then
    ...
    While Not rs.EOF
        ...
        scomm = UCase(rs("CATF_Nom").Value) & " - " & UCase(rs("FOR_Lib").Value) & vbCrLf & titre & vbCrLf & vtxt
...
Private Function evalue_action(ByRef r_bnew) As Integer
    ...
    While Not rs.EOF
        ...
        If g_bindic(INDIC_ACTION) Then
            If Not IsNull(rs("NbAction").Value) Then
        ...
        If g_bindic(INDIC_MODIF) Then
            If Not IsNull(rs("NbModif").Value) Then
        ...
        If g_bindic(INDIC_AR) Then
            If Not IsNull(rs("NbDiff").Value) Then
        ...
        If g_bindic(INDIC_INFO) Then
            If Not IsNull(rs("NbInfo").Value) Then
...
Private Function verif_modifications(ByVal v_lnbmod As Long) As Integer
    ...
    Dim sql As String, snumutil As String
    Dim lnb As Long
...
Private Sub cmd_Click(Index As Integer)
    ...
    If Index < CMD_DET Then
        ...
        Else
            g_position = g_position + 1
            If g_position > g_loffset + NB_LIMIT Then
                g_loffset = g_loffset + NB_LIMIT
                Call charger_kform
            End If
        End If
        ...
        For i = 0 To nblig - 1
            ...
            If (p_indic(g_position).stype = "kgdr" And i > 0) Or (p_indic(g_position).stype = "kform" And i > 1) Then
    ...
    'Accès au détail KD
    ElseIf Index = CMD_KD Then
        tb = Split(p_nomini, "\")
        Shell "C:\KaliDoc\Lance.exe c:\kalidoc;kalidoc;" & tb(UBound(tb)) & ";CONNEXION=" & p_numUtil, vbNormalFocus
     
    'Accès au détail KR
    ElseIf Index = CMD_KR Then
        'Shell "C:\KaliDoc\Lance.exe c:\kalidoc;kaligdr;kaligdr.ini;CONNEXION=" & p_numUtil, vbNormalFocus
        Shell "C:\kalidoc\Lance.exe c:\kalidoc;kaligdr;kaligdr.ini;" & p_numUtil, vbNormalFocus
    
    ' Accès à KaliWeb
    ElseIf Index = CMD_KW Then
...
Private Sub Form_Load()
    ...
    'Gestion du timer
    If GetSetting(App.EXEName, "Section", "Rafraichissement") <> "" Then
        g_ltimer = GetSetting(App.EXEName, "Section", "Rafraichissement")
    Else
        g_ltimer = 60000
        SaveSetting App.EXEName, "Section", "Rafraichissement", 60000
    End If
   ' Timer1.Enabled = True

    'Vérifier si application disponible
    If FICH_FichierExiste("c:\kalidoc\kalidoc.exe") Then
        cmd(CMD_KD).Visible = True
    Else
        cmd(CMD_KD).Visible = False
    End If
        
    If FICH_FichierExiste("c:\kalidoc\kaligdr.exe") Then
        cmd(CMD_KR).Visible = True
    Else
        cmd(CMD_KR).Visible = False
    End If
...
test:
        ...
        Else
            ...
            If Odbc_Init(p_stype_bdd, p_nom_bdd, True) = P_OK Then
                GoTo test
            End If
