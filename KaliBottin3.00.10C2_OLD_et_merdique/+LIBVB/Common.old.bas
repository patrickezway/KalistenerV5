...
err_copy:
    MsgBox "Impossible de copier " & v_nomfich_src & " dans " & v_nomfich_dest & "." & vbCr & vbLf & "Err=" & Err.Number & " " & Err.Description, _
            vbCritical + vbOKOnly, "Fichier (FICH_CopierFichier)"
    On Error GoTo 0
    FICH_CopierFichier = P_ERREUR
    Exit Function
...
Public Function FICH_CreerRepComp(ByVal v_nomrep As String, _
    ...
    If s <> "" Then
        If v_bconfirm_si_existe Then
            reponse = MsgBox("Le répertoire '" & v_nomrep & "' existe déjà." & vbCr & vbLf _
                           & "Confirmez-vous votre choix ?", vbQuestion + vbYesNo, "")
...
err_mkdir:
    MsgBox "Erreur création du répertoire '" + sdir & "'" & vbLf & vbCr & "Erreur=" & Err.Number & vbLf & vbCr & Err.Description, _
            vbOKOnly + vbCritical, "Fichier (FICH_CreerRepComp)"
    FICH_CreerRepComp = P_ERREUR
    Exit Function
...
err_kill:
    If v_ya_mess Then
        MsgBox "Impossible d'effacer " & v_nomfich & "." & vbCr & vbLf & "Err=" & Err.Number & " " & Err.Description, _
                vbCritical + vbOKOnly, "Fichier (FICH_EffacerFichier)"
    End If
    On Error GoTo 0
...
err_open:
    MsgBox "Erreur Open " & v_nomfich & vbCr & vbLf & Err.Number & vbLf & vbCr & Err.Description, _
            vbOKOnly + vbCritical, "Fichier (FICH_OuvrirFichier)"
...
err_rename:
    MsgBox "Impossible de renommer '" & v_nomfich_src & "' en '" & v_nomfich_dest & "'." & vbCr & vbLf _
         & "Err=" & Err.Number & " " & Err.Description, vbCritical + vbOKOnly, "Fichier (FICH_RenommerFichier)"
    On Error GoTo 0
...
err_shell:
    Call MsgBox("Impossible d'exécuter : " & v_scmd & vbCr & vbLf & Err.Number & " " & Err.Description, _
...
Public Function P_lister_fich_rep(ByVal v_path As String, ByRef v_liste() As String, ByVal v_rep As Boolean) As Long
'******************************************************************************************
...
Public Function STR_GetChamp(ByVal v_str As String, _
    ...
    For i = 0 To v_nochp - 1
        ...
        If pos = 0 Then
            ...
            Exit Function
        End If
        ' s = Mid$(s, pos + 1) ' ORIGINALE
        s = Mid$(s, pos + Len(v_sep)) ' Pour pouvoir utiliser v_sep avec plus qu'1 seul caractère
    Next i
...
Public Function STR_Crypter2(ByVal v_passwd As String, _
                            ByVal v_str As String) As String

    Dim s_code As String, sc As String, str As String, tbl_scode() As String
    Dim str_c As String
    Dim i As Integer, pos As Integer, n As Integer

    s_code = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzàâéèêôùû1234567890|"
    s_code = s_code + s_code

    str = STR_Remplacer(v_str, "\t", "[tab]")
    str = STR_Remplacer(str, "\n", "[new]")
    str = STR_Remplacer(str, "\r", "[ret]")
    
    ReDim tbl_scode(1 To Len(v_passwd)) As String
    For i = 1 To Len(v_passwd)
        sc = Mid$(v_passwd, i, 1)
        pos = InStr(s_code, sc)
        tbl_scode(i) = Mid$(s_code, pos)
    Next i
    ...
    For i = 1 To Len(str)
        sc = Mid$(str, i, 1)
        pos = InStr(s_code, sc)
        str_c = str_c + Mid$(tbl_scode(n), pos, 1)
        If n = Len(v_passwd) Then
            n = 1
        Else
            n = n + 1
        End If
...
Public Function STR_Phonet(ByVal v_str As String) As String
    ...
    str = UCase(str)
    If Right$(str, 1) = "S" Or Right$(str, 1) = "X" Then
...
Public Sub STR_PutChamp(ByRef r_str As String, _
                        ByVal v_sep As String, _
...
Public Function STR_LaisserUnSeulBlanc(ByVal v_str As String) As String

    Dim deja_bl As Boolean
    Dim s1 As String, s2 As String, sc As String
