...
' call msgbox("On force l'adresse Kalitech")
'v_adrdest = "kalitech@kalitech.fr"
    ...
    If Len(v_filename) > 0 Then
        If Len(Dir$(v_filename)) > 0 Then
            ...
            If InStrRev(v_filename, "\") > 0 Then
                MAPIMessage.AttachmentName = Mid$(v_filename, InStrRev(v_filename, "\") + 1)
            Else
                MAPIMessage.AttachmentName = v_filename
            End If
...
'    MAPIMessage.action = action
    MAPIMessage.Send Not v_BackGround
...
lab_erreur:
    ...
    Else
        ...
        ' Outlook demande confirmation et on répond NON
        If Err = 32026 Then
            On Error GoTo 0
            EnvoiMessage = -1
            Exit Function
